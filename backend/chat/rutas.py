...
def create_chat(chat: schema.ChatCreate, db: Session = Depends(get_db), current_user: dict = Depends(verify_token)):
    return crud.create_chat(db=db, chat=chat)

@router.get("/{chat_id}", response_model=schema.Chat, status_code=status.HTTP_200_OK)
def read_chat(chat_id: int, db: Session = Depends(get_db), current_user: dict = Depends(verify_token)):
...
def delete_chat(chat_id: int, db: Session = Depends(get_db), current_user: dict = Depends(verify_token)):
